...
class QueenTest : AnnotationSpec() {
  ...
  fun `Queen movement on empty board multiple squares up`() {
    ...
    val move = Move(Position('a', 2), Position('a', 8))
  ...
  fun `Queen movement on empty board multiple squares down right`() {
    val board = Board(FENData("Q6K/8/8/8/8/8/8/8", 'w', ""))
    val move = Move(Position('a', 8), Position('h', 1))
    board.turn = Color.WHITE
    board.makeMove(move)
  ...
  fun `Queen movement on empty board multiple squares up left`() {
    ...
    val move = Move(Position('h', 1), Position('d', 5))
    board.turn = Color.WHITE
    board.makeMove(move)
  ...
  fun `Queen movement on empty board multiple squares up right`() {
    ...
    val move = Move(Position('d', 5), Position('g', 8))
    board.turn = Color.WHITE
    board.makeMove(move)
  ...
  fun `Queen movement on empty board multiple squares down left`() {
    ...
    val move = Move(Position('g', 8), Position('a', 2))
    board.turn = Color.WHITE
    board.makeMove(move)
    assertThat(board.generateFENBoardString()).isEqualTo("7K/8/8/8/8/8/Q7/8")
  }
