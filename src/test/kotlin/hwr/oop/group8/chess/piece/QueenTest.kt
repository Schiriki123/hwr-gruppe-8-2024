...
class QueenTest : AnnotationSpec() {
  ...
  fun `Queen movement on empty board multiple squares up`() {
    val board = Board(FENData("7K/8/8/8/8/8/Q7/8", 'w', ""))
    val singleMove = SingleMove(Position(File.A, Rank.TWO), Position(File.A, Rank.EIGHT))
  ...
  fun `Queen movement on empty board multiple squares down right`() {
    val board = Board(FENData("Q6K/8/8/8/8/8/8/8", 'w', ""))
    val singleMove = SingleMove(Position(File.A, Rank.EIGHT), Position(File.H, Rank.ONE))
    board.turn = Color.WHITE
    board.makeMove(singleMove)
  ...
  fun `Queen movement on empty board multiple squares up left`() {
    val board = Board(FENData("7K/8/8/8/8/8/8/7Q", 'w', ""))
    val singleMove = SingleMove(Position(File.H, Rank.ONE), Position(File.D, Rank.FIVE))
    board.turn = Color.WHITE
    board.makeMove(singleMove)
  ...
  fun `Queen movement on empty board multiple squares up right`() {
    val board = Board(FENData("7K/8/8/3Q4/8/8/8/8", 'w', ""))
    val singleMove = SingleMove(Position(File.D, Rank.FIVE), Position(File.G, Rank.EIGHT))
    board.turn = Color.WHITE
    board.makeMove(singleMove)
  ...
  fun `Queen movement on empty board multiple squares down left`() {
    val board = Board(FENData("6QK/8/8/8/8/8/8/8", 'w', ""))
    val singleMove = SingleMove(Position(File.G, Rank.EIGHT), Position(File.A, Rank.TWO))
    board.turn = Color.WHITE
    board.makeMove(singleMove)
    assertThat(board.generateFENBoardString()).isEqualTo("7K/8/8/8/8/8/Q7/8")
  }
  ...
  fun `Diagonal Queen movement with blocked path`() {
    val board = Board(FENData("Q7/8/8/8/4R3/8/8/K7", castle = ""))
